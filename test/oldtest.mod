...
TYPE
  ...
  intrecdesc = RECORD
               ...
               END;
VAR
  i: INTEGER;        (* Keep this as the first global variable - allocated at highest address *)
  j: intrec;
  k: intrecdesc;
  s: ARRAY 10 OF CHAR;
  c: CHAR;
  ...
  s8: ARRAY 10 OF CHAR;
  memstart: INTEGER;
  memlimit: INTEGER; (* Keep this as the last global variable - allocated at lowest address *)
...
PROCEDURE whn(i, n: INTEGER); BEGIN IF n>1 THEN whn(i DIV 16, n-1) END;  wh1(i MOD 16)    END whn;
...
BEGIN
  ...
  *)
  ...
  ws("Address of c8: CHAR8            "); wh(SYSTEM.ADR(c8)); wl;
  ws("Address of s8: CHAR8            "); wh(SYSTEM.ADR(s8)); wl;
  ...
  wsl("VAR dump:");
  dump(2, SYSTEM.ADR(memlimit), SYSTEM.ADR(i) + SYSTEM.SIZE(INTEGER) - SYSTEM.ADR(memlimit));
