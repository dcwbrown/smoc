...
PROCEDURE write_import_name(offset: INTEGER; name: ARRAY OF CHAR);
BEGIN
  Files.Set(rider, out, idata_fadr + hint_rva + 2 + offset);
  Files.WriteByteStr(rider, name);
END write_import_name;

BEGIN (* Write_idata_section *)
  Files.Set(rider, out, idata_fadr);

  (* Write import Directory Entry - Kernel32.dll *)
  Files.WriteCard32(rider, idata_rva + table_rva);  (* RVA of import lookup table *)
  Files.WriteCard32(rider, 0);                      (* Time/Date stamp (none) *)
  Files.WriteCard32(rider, 0);                      (* Forwarder chain (none) *)
  Files.WriteCard32(rider, idata_rva + name_rva);   (* Name of DLL being imported from *)
  Files.WriteCard32(rider, data_rva);               (* RVAs of imported addresses (start of data section) *)

  (* Write import lookup table (also written to start of data section) *)
  Files.Set(rider, out, idata_fadr + table_rva);
  FOR i := 0 TO table_len - 2 DO
    Kernel32Table[i] := idata_rva + hint_rva + 32 * i;  (* Lookup by name given in hint table *)
    Files.WriteInt(rider, Kernel32Table[i])
  END;
  Kernel32Table[table_len - 1] := 0;

  (* Write DLL name being imported from *)
  Files.Set(rider, out, idata_fadr + name_rva);
  Files.WriteByteStr(rider, 'KERNEL32.DLL');

  (* Write hint table of imported entry point names *)
  write_import_name(0,   'GetProcAddress');
  write_import_name(32,  'LoadLibraryW');
  write_import_name(64,  'ExitProcess');
  write_import_name(96,  'GetModuleHandleExW');
  write_import_name(128, 'AddVectoredExceptionHandler')
...
BEGIN
  ...
  IF bss_size > 0 THEN
    Write_SectionHeader('.bss', -1073741696, bss_rva, 0, bss_size, 0)
  END;
  Write_SectionHeader('.data',  -1073741760, data_rva,  data_rawsize,  data_size,  data_fadr);
  Write_SectionHeader('.text',  60000020H,   code_rva,  code_rawsize,  code_size,  code_fadr);
  Write_SectionHeader('.idata', -1073741760, idata_rva, 200H,          130H,       idata_fadr);
  Write_SectionHeader('.reloc', 42000040H,   reloc_rva, 200H,          12,         reloc_fadr);
  Write_SectionHeader('.pdata', 40000040H,   pdata_rva, pdata_rawsize, pdata_size, pdata_fadr);
  Write_SectionHeader('.edata', 40000040H,   edata_rva, edata_rawsize, edata_size, edata_fadr);

  (* Compiler-specifics data *)
  Files.Set(rider, out, 400H - 40);
  Files.WriteInt(rider, exportAdrTable_rva);
  Files.WriteInt(rider, code_rva);
  Files.WriteInt(rider, pdata_rva+32);
...
BEGIN
  ...
  Write_code_section(code);
  Write_idata_section;
  Write_data_section;
  Write_reloc_section;
  Write_pdata_section(debug);
  Write_edata_section;
  Write_PEHeader;
